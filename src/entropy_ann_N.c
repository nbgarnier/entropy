...
/* 2022-04-14 : forked from "compute_entropy_ann", see brouillon                        */
/****************************************************************************************/
int compute_entropy_ann_N(double *x, int npts, int m, int p, int tau, 
...
{	register int j; 
...
#ifdef DEBUGp
    debug_trace("[compute_entropy_ann_N] signal x", x, npts, m, p, stride, k);
#endif
    ...
    *S = NAN;
    ...
    if ((m<1) || (p<1))     return(printf("[compute_entropy_ann_N] : m and p must be at least 1 !\n"));
    if ((tau<1))            return(printf("[compute_entropy_ann_N] : tau must be at least 1 !\n"));
    if ((k<1))              return(printf("[compute_entropy_ann_N] : k must be at least 1 !\n"));
    ...
    {   if (sp.type>=3) shuffle_perm(perm_pts);
        ...
        if (USE_PTHREAD>0) 
        {   S_tmp = compute_entropy_nd_ann_threads(x_new, N_eff, n, k, get_cores_number(GET_CORES_SELECTED));
        }
        else
            S_tmp = compute_entropy_nd_ann        (x_new, N_eff, n, k);
...
*****************************************************************************************/
int compute_relative_entropy_ann_N(double *x, int nx, double *y, int ny, int mx, int my, int px, int py, int tau, 
...
{   double  *x_new, *y_new;
    ...
    *H=NAN;
    ...
    if (k<1)              return(printf("[compute_relative_entropy_ann_N] : k has to be equal or larger than 1.\n"));
    ...
    if (sp_x.N_eff < 2*k) return(printf("[compute_relative_entropy_ann_N x] : N_eff=%d is too small compared to k=%d)\n", sp_x.N_eff, k));
    if (sp_y.N_eff < 2*k) return(printf("[compute_relative_entropy_ann_N y] : N_eff=%d is too small compared to k=%d)\n", sp_y.N_eff, k));
...
/* 2020-02-26 : added output of standard deviation                                  */
/************************************************************************************/
int compute_mutual_information_ann_N(double *x, double *y, int nx, int mx, int my, int px, int py, int tau, 
...
#ifdef DEBUG
     debug_trace("[compute_mutual_information_ann_N] signal x", x, nx, mx, px, stride, k);
     debug_trace("[compute_mutual_information_ann_N] signal y", y, nx, my, py, stride, k);
#endif
     
	if ((mx<1) || (my<1)) return(printf("[compute_mutual_information_ann_N] : mx and my must be at least 1 !\n"));
	if ((px<1) || (py<1)) return(printf("[compute_mutual_information_ann_N] : px and py must be at least 1 !\n"));
	if ((tau<1))          return(printf("[compute_mutual_information_ann_N] : tau must be at least 1 !\n"));
    if (k<1)              return(printf("[compute_mutual_information_ann_N] : k has to be at least 1.\n"));
    
    // additional checks and auto-adjustments of parameters:
    N_real_max = set_sampling_parameters(nx, pp, tau, &sp, "compute_mutual_information_ann_N");
    if (N_real_max<1)     return(printf("[compute_mutual_information_ann_N] : aborting !\n"));
    if (sp.N_eff < 2*k)   return(printf("[compute_mutual_information_ann_N] : N_eff=%d is too small compared to k=%d)\n", sp.N_eff, k));
...
#ifdef DEBUG
         debug_trace("[compute_mutual_information_ann_N] signal x_new", x_new, sp.N_eff, n, px, stride, k);
#endif
 ...
 *************************************************************************************/
int compute_partial_MI_ann_N(double *x, double *y, double *z, int npts, int *dim, int tau, 
                            int tau_Theiler, int N_eff, int N_realizations, int k, double *I1, double *I2)
{	register int j, shift, pp, mx=dim[0], my=dim[1], mz=dim[2], px=dim[3+0], py=dim[3+1], pz=dim[3+2];
    ...
    *I1=NAN; *I2=NAN;
	...
	if ((px<1) || (py<1) || (pz<1)) return(printf("[compute_partial_MI_ann_N] : embedding dimensions must be at least 1 !\n"));
    if ((mx<1) || (my<1) || (mz<1)) return(printf("[compute_partial_MI_ann_N] : initial dimensions of data must be at least 1 !\n"));
    if (tau<1)                      return(printf("[compute_partial_MI_ann_N] : tau has to be equal or larger than 1.\n"));
    if (k<1)                        return(printf("[compute_partial_MI_ann_N] : k must be at least 1 !\n"));
          
	pp         = (px>py) ? px : py;     // who has the largest past ?
	pp         = (pp>pz) ? pp : pz;
    
    // checks and auto-adjustments of sampling parameters:
    N_real_max = set_sampling_parameters(npts, pp, tau, &sp, "compute_partial_MI_ann_N");
    if (N_real_max<1)     return(printf("[compute_partial_MI_ann_N] : aborting !\n"));
    if (sp.N_eff < 2*k)   return(printf("[compute_partial_MI_ann_N] : N_eff=%d is too small compared to k=%d)\n", sp.N_eff, k));

	x_new  = (double*)calloc(n*sp.N_eff, sizeof(double));
    if (x_new==NULL) return(printf("[compute_partial_MI_ann_N] : memory allocation problem !\n"
	...
	*I2  = avg2;    	last_std2 = sqrt(var2);
    ...
    free_perm(perm_real);    free_perm(perm_pts);
	return(nb_errors);
} /* end of function "compute_partial_MI_ann_N" */
