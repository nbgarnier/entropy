 ...
 *
 *
 *  2025-02-13 : split into 2 functions for cross-entropy, and relative entropy
 */
...
/**************************************************************************************
 * computes cross entropy H(x||y), using nearest neighbor statistics
...
{   register int core, npts_eff_min, n_total=0;
    ...
    {   my_arguments[core].core    = core;
        ...
        pthread_create(&thread[core], NULL, threaded_cross_entropy_func, (void *)&my_arguments[core]);
    }
    for (core=0; core<nb_cores; core++)
    ...
    // sanity check:
    if (n_total!=nx) printf("[compute_cross_entropy_2xnd_ann_threads] TROUBLE! npts altered!\n");
    ...
    return(h);
} /* end of function "compute_cross_entropy_2xnd_ann_threads" ******************************/



/****************************************************************************************/
/* function to be used by "compute_relative_entropy_2xnd_ann_threads"                   */
/*                                                                                      */
/* 2021-11-26  first multi-threads version                                              */
/* 2025-02-13  fork from "cross" (which was named "relative")                           */
/****************************************************************************************/
void *threaded_relative_entropy_func(void *ptr)
{   struct thread_args  *args = (struct thread_args *)ptr; // cast arguments to the usable struct
    struct thread_output *out = calloc(1, sizeof(struct thread_output)); // allocate heap memory for this thread's results
    register int i, j;
    double epsilon_x=0.0, epsilon_y=0.0, l_hs=0.0;
    int core     = args->core,
        i_start  = args->i_start,
        i_end    = args->i_end,
        nx       = args->nx,
...
/**************************************************************************************
 * computes complete relative entropy H(x||y), using nearest neighbor statistics
 * this is derived from: Leonenko et al, Annals of Statistics 36 (5) pp2153–2182 (2008)
 * page 2163
 *
 * this version is for n-dimentional systems, and uses ANN library with kd-tree
...
{   register int core, npts_eff_min, n_total=0;
    ...
    init_ANN_RE(nx, ny, n, k, get_cores_number(GET_CORES_SELECTED));
